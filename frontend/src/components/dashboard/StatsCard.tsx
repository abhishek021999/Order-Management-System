...
const StatsCard: React.FC<Props> = ({ title, value, subtitle, icon, iconBg, trend, gradient, onClick }) => (
  <div
    ...
    onClick={onClick}
  >
    <div className={`stat-icon ${iconBg} shadow-inner-sm [&>svg]:w-4 [&>svg]:h-4 sm:[&>svg]:w-[22px] sm:[&>svg]:h-[22px]`}>{icon}</div>
    <div className="min-w-0 flex-1">
      <p className="text-[10px] sm:text-xs font-bold text-slate-400 uppercase tracking-widest truncate">{title}</p>
      <p className="text-base sm:text-[1.6rem] font-extrabold text-slate-900 mt-0.5 leading-tight tracking-tight truncate">{value}</p>
      {subtitle && <p className="hidden sm:block text-xs text-slate-400 mt-1 leading-none">{subtitle}</p>}
      {trend && (
